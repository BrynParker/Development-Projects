...
function CPANEL:Paint(w, h) --inventory panels
	local parent = self:GetParent()
		...
		--[[
		surface.SetDrawColor( 17, 31, 35, 60 )
	...
	if self:IsHovered() then
		...
		surface.SetMaterial( Material("mafiarp/menu/icon_stripes.png") )
		surface.DrawTexturedRect( 0, 0, w, h )
	end
...
PANEL = {}
	...
	function PANEL:buildSlots()
		...
		local function PaintSlot(slot, w, h)
			surface.SetDrawColor(45, 45, 45, 80)
    ...
    function PANEL:Paint(w, h) --рисуем панель инвентаря
    	draw.DrawText("Inventory", "InterThinLarge", ScrW() * 0.016, ScrH() * 0.02, Color(182, 182, 182, 220), TEXT_ALIGN_LEFT )	
