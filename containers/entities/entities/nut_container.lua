...
if (SERVER) then
	function ENT:Initialize()
		...
		self:setNetVar("name", "Storage Container")
		self.receivers = {}
	...
	function ENT:OpenInv(activator)
		...
		activator:setAction("Opening...", OPEN_TIME, function()
			if (activator:GetPos():Distance(self:GetPos()) <= 100) then
				...
				self:EmitSound(def.opensound or "interface/inv_drop.ogg")
			end
		end)
	end
	...
	function ENT:Use(activator)
		if not self.placed and (activator:getChar():getID() == self.owner or activator:IsAdmin()) and self:IsValid() then
			if activator:KeyDown(IN_WALK) then
				...
				activator:doStaredAction(self, function()
					if (!activator:getChar():getInv():add(self.item.uniqueID)) then
						activator:notify("You do not have enough space in your inventory")
						return
					else
						self:Remove()
					end	
			...
			self:EmitSound("interface/inv_slot.ogg")
		...
		if self.placed and activator:KeyDown(IN_WALK) and activator:getChar():getID() == self.owner and self:IsValid() then
			...
			elseif self.money ~= 0 then
				activator:notify("You cannot move a container with money in it")
				return
			elseif self.lockedid ~= nil then
				activator:notify("You cannot move a container while it is locked")
				return
			else
				activator:notify("You cannot move a container with items in it")
				return
			end
		end
		local inventory = self:getInv()
